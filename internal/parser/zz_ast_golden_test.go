...
func collectExpr(path string, e Expression, rows *[]row) {
	...
	case *RndExpr:
		*rows = append(*rows, row{path, "RndExpr", ""})
		collectExpr(path+"/Expr", ex.Expr, rows)

	case *SinExpr:
		*rows = append(*rows, row{path, "SinExpr", ""})
		collectExpr(path+"/Expr", ex.Expr, rows)

	case *CosExpr:
		*rows = append(*rows, row{path, "CosExpr", ""})
		collectExpr(path+"/Expr", ex.Expr, rows)

	case *TanExpr:
		*rows = append(*rows, row{path, "TanExpr", ""})
		collectExpr(path+"/Expr", ex.Expr, rows)

	case *AtnExpr:
		*rows = append(*rows, row{path, "AtnExpr", ""})
		collectExpr(path+"/Expr", ex.Expr, rows)
	...
	case *SpcExpr:
		*rows = append(*rows, row{path, "SpcExpr", ""})
		collectExpr(path+"/Expr", ex.Expr, rows)
	}
}
