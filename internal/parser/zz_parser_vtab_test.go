...
func TestParse_VTAB_Statement(t *testing.T) {
	tests := []struct {
		name       string
		source     string
		lineCount  int
		...
		{
			name: "VTAB same line with PRINT",
			source: `
10 VTAB 10: PRINT "HELLO"
`,
			lineCount: 1,
			assertFunc: func(t *testing.T, prog *Program) {
				line := prog.Lines[0]
				...
				_, ok := line.Stmts[0].(*VTabStmt)
				testutils.Equal(t, "first stmt is VTabStmt", ok, true)
...
`,
			...
			assertFunc: func(t *testing.T, prog *Program) {
				...
				_, ok := line10.Stmts[0].(*VTabStmt)
				testutils.Equal(t, "line 10 is VTabStmt", ok, true)
...
`,
			...
			assertFunc: func(t *testing.T, prog *Program) {
				...
				_, ok = line20.Stmts[0].(*VTabStmt)
				testutils.Equal(t, "line 20 first stmt is VTabStmt", ok, true)
