...
func TestParse_SQR_Expressions(t *testing.T) {
	tests := []struct {
		name      string
		source    string
		lineCount int
		...
		{
			name:      "SQR with positive integer",
			source:    `10 PRINT SQR(3)`,
			lineCount: 1,
			assertFn: func(t *testing.T, prog *Program) {
				...
				sqrExpr, ok := printStmt.Exprs[0].(*SqrExpr)
				testutils.True(t, "expression is SqrExpr", ok)

				num, ok := sqrExpr.Expr.(*NumberLiteral)
				testutils.True(t, "SQR argument is NumberLiteral", ok)
				testutils.Equal(t, "number value", num.Value, 3)
			},
		},
		{
			name:      "SQR with negative integer",
			source:    `10 PRINT SQR(-3)`,
			lineCount: 1,
			assertFn: func(t *testing.T, prog *Program) {
				sqrExpr := prog.Lines[0].Stmts[0].(*PrintStmt).Exprs[0].(*SqrExpr)

				prefix, ok := sqrExpr.Expr.(*PrefixExpr)
		...
		{
			name:      "SQR with positive float",
			source:    `10 PRINT SQR(1.75)`,
			lineCount: 1,
			assertFn: func(t *testing.T, prog *Program) {
				printStmt, ok := prog.Lines[0].Stmts[0].(*PrintStmt)
				testutils.True(t, "is PrintStmt", ok)
		...
		{
			name:      "SQR with float variable",
			source:    `10 PRINT SQR(A)`,
			lineCount: 1,
			assertFn: func(t *testing.T, prog *Program) {
				sqrExpr := prog.Lines[0].Stmts[0].(*PrintStmt).Exprs[0].(*SqrExpr)

				_, ok := sqrExpr.Expr.(*Identifier)
				testutils.True(t, "SQR arg is Identifier", ok)
			},
		},
		{
			name:      "SQR with expression A * 3.74",
			source:    `10 PRINT SQR(A * 3.74)`,
			lineCount: 1,
			assertFn: func(t *testing.T, prog *Program) {
				sqrExpr := prog.Lines[0].Stmts[0].(*PrintStmt).Exprs[0].(*SqrExpr)

				infix, ok := sqrExpr.Expr.(*InfixExpr)
		...
		{
			name:      "SQR with integer variable",
			source:    `10 PRINT SQR(I%)`,
			lineCount: 1,
			assertFn: func(t *testing.T, prog *Program) {
				sqrExpr := prog.Lines[0].Stmts[0].(*PrintStmt).Exprs[0].(*SqrExpr)

				id, ok := sqrExpr.Expr.(*Identifier)
				testutils.True(t, "arg is Identifier", ok)
				testutils.Equal(t, "identifier name", id.Name, "I%")
			},
		},
		{
			name:      "SQR with negative integer variable",
			source:    `10 PRINT SQR(-I%)`,
			lineCount: 1,
			assertFn: func(t *testing.T, prog *Program) {
				sqrExpr := prog.Lines[0].Stmts[0].(*PrintStmt).Exprs[0].(*SqrExpr)
		...
		{
			name:      "SQR with mixed expression I% * A",
			source:    `10 PRINT SQR(I% * A)`,
			lineCount: 1,
			assertFn: func(t *testing.T, prog *Program) {
				sqrExpr := prog.Lines[0].Stmts[0].(*PrintStmt).Exprs[0].(*SqrExpr)
		...
		{
			name:      "SQR with nested expression -(I% * A)",
			source:    `10 PRINT SQR(-(I% * A))`,
			lineCount: 1,
			assertFn: func(t *testing.T, prog *Program) {
				sqrExpr := prog.Lines[0].Stmts[0].(*PrintStmt).Exprs[0].(*SqrExpr)
