...
func TestParse_FLASH_Multiline(t *testing.T) {
	source := `
10 NORMAL
20 PRINT "HELLO"
30 FLASH
...
`
	...
	_, ok = line30.Stmts[0].(*FlashStmt)
	testutils.True(t, "stmt is FlashStmt", ok)
...
func TestParse_FLASH_WithPrintChain(t *testing.T) {
	source := `
10 NORMAL
20 PRINT "HELLO ";
30 FLASH
...
`
	...
	_, ok = prog.Lines[2].Stmts[0].(*FlashStmt)
	testutils.True(t, "line 30 FLASH", ok)
...
func TestParse_FLASH_SingleLine(t *testing.T) {
	source := `
...
`
	...
	_, ok = line.Stmts[2].(*FlashStmt)
	testutils.True(t, "stmt 2 FLASH", ok)
