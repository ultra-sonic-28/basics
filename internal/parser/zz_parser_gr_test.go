...
func TestParse_GR_Statement(t *testing.T) {
	source := `
10 GR
`
	...
	// --- Type assertion ---
	gr, ok := stmt.(*GrStmt)
	testutils.Equal(t, "statement is GrStmt", ok, true)

	// --- Bonus : vérification position ---
	testutils.Equal(t, "line stored", gr.Line, 2)
}

func TestParse_IF_THEN_GR(t *testing.T) {
	source := `
10 IF A=1 THEN GR
`
	...
	_, ok = ifStmt.Then[0].(*GrStmt)
	testutils.Equal(t, "then stmt is GrStmt", ok, true)

	// --- ELSE block absent ---
	testutils.Equal(t, "else block empty", len(ifStmt.Else), 0)
}

func TestParse_GR_COLON_PRINT(t *testing.T) {
	source := `
10 GR: PRINT "OK"
`
	...
	// --- Statement 1 : GR ---
	_, ok := line.Stmts[0].(*GrStmt)
	testutils.Equal(t, "stmt[0] is GrStmt", ok, true)
